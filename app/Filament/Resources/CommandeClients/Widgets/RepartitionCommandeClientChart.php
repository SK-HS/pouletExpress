...
namespace App\Filament\Resources\RepartitionCommandeClientChart\Widgets;
...
class RepartitionCommandeClientChart extends ChartWidget
{
    protected ?string $heading = 'Gaphe des Repartitions des Produits Commandés par Clients';
   ...
   protected function getData(): array
{
    ...
    // 1. Récupérer tous les clients triés par CA décroissant
    $ventes = CommandeClient::query()
        ...
        if ($autres->count() > 0) {
            $labels[] = 'Autres (' . $autres->count() . ' clients)';
    ...
    return [
        'datasets' => [
            [
                'label' => 'Commandes (FCFA)',
                'data' => $montants,
                 'backgroundColor' => '#10b93a', // Vert
                    'borderRadius' => 4,
            ],
            [
