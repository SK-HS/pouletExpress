...
<main class="lg:ml-64 p-4 lg:p-margin-desktop grid grid-cols-12 gap-4 lg:gap-gutter animate-in fade-in duration-500 pb-28">
        ...
        <section class="relative bg-surface-container-high rounded-xl overflow-hidden h-[350px] lg:h-[450px] shadow-sm border border-outline-variant/30">
            ...
            <div id="livreur-card"
                ...
                <div class="flex-1 min-w-0">
                    <p title="{{ $livraison?->livreur?->telephone }}" class="font-body-md-bold text-sm lg:text-base truncate" id="livreur-nom">
    ...
    <div class="col-span-12 lg:col-span-4 order-2 lg:order-1 flex flex-col gap-4 lg:gap-gutter">
                ...
                <div class="min-w-0">
                    ...
                    <h3 class="font-headline-md text-base lg:text-headline-md truncate">{{ $commande->client?->nom ?? '—' }}</h3>
                     <p class="text-xs text-on-surface-variant mt-0.5 "> 
                        <span class="material-symbols-outlined text-on-primary" >phone</span>
                         {{ $commande->client?->telephone }}</p>
                    <p class="text-xs text-on-surface-variant mt-0.5" >
                        <span class="material-symbols-outlined text-on-primary" style="font-variation-settings: 'FILL' 1;" >home</span>
                        {{ $commande->client?->adresse }}</p>
        ...
        {{-- Timeline --}}
        <section class="bg-surface-container-lowest rounded-xl p-4 lg:p-card-padding shadow-sm border border-outline-variant/30">
            <h4 class="font-headline-md text-base lg:text-headline-md mb-4 lg:mb-6">Suivi d'activité
                @if($commande->cmmd_livre_fournisseur==0)
                <form action="{{ route('Commande-Livree-Fournisseur',$commande->id) }}" method="POST" class="flex items-center gap-4 px-4 py-3 rounded-xl bg-secondary transition-all duration-150 hover:bg-error-container text-on-surface-variant">
                    @csrf
                    @method('PUT')
                <button>
              <span class="material-symbols-outlined">rotate_left</span>
                 <span class="font-body-md text-body-md">Marquer comme livrée</span>
                </button>

          </form>
          @else
          <button class="flex items-center gap-4 px-4 py-3 rounded-xl bg-primary transition-all duration-150 hover:bg-error-container text-on-surface-variant">
                <span class="material-symbols-outlined">check</span>
                <span class="font-body-md text-body-md">Comme livrée</span>
                </button>
          @endif
            </h4>
...
document.addEventListener('DOMContentLoaded', function () {
   ...
   const pollingUrl = '{{ route("Suivi-Position-Commande", $commande->id) }}';
    ...
    document.getElementById('btn-recenter').addEventListener('click', function () {
        if (!map) {
            alert('La carte n\'est pas encore chargée. Patientez quelques secondes.');
            return;
        }

        if (livreurMarker) {
            // Priorité : recentrer sur le livreur s'il a une position connue
            map.setView(livreurMarker.getLatLng(), 15);
        } else if (bounds.length > 0) {
            // Sinon, recentrer sur les points fixes disponibles (fournisseur/client)
            if (bounds.length > 1) {
            ...
            alert('Aucune position disponible pour le moment.');
        }
    });
